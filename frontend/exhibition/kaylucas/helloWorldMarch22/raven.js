  ...
  display() {
    if (this.greyToggle != greyScale) {
      ...
      if (greyScale) {
        ...
        this.tailColor = color(0);
        this.eyeRing = color(192)
        this.uBeakColor = color(90)
        this.lBeakColor = color(120)
      }
      if (!greyScale) {
        this.headColor = color(0); // 86, 86, 86
        this.bodyColor = color(0); // 200, 150,  170); 
    ...
    fill(this.uBeakColor) //this.uBeakColor = color(86, 86, 86)
    if (this.bw) {
      noFill();
    }
    rotateZ(-HALF_PI + .05) // rotate beak down
    cone(10, 70, this.detail, false);// beak size scale //12, 70
    ...
    fill(this.lBeakColor); //this.lBeakColor = color(120, 120, 120)
    if (this.bw) {
      noFill();
    }
    cone(8, 70, this.detail, false);// beak size scale //12, 70
    pop();

    rotateZ(HALF_PI - .2)
    translate(-36, -10, 0) // nose ellipsoid
    //ellipsoid(20, 12, 16, this.detail, this.detail) // red nose ellipsoid
    pop()
    push()
    fill(this.eyeRing)
    stroke(this.eyeRing) // torus  this.eyeRing = color(16, 105, 192)
    if (this.bw) {
      noFill();
      stroke(255);
    }
