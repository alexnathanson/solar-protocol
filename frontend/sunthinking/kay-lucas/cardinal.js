class cardinal {
  constructor(_w, _YRot, _x, _y, _z) {
    ...
    this.wingColor = color(150, 150, 150);
    this.neckColor = color(255);
    this.feetColor = color(100, 100, 100);
    this.legColor = color(255);
    this.tailColor = color(150, 150, 150);
    this.beakColor = color(236, 230, 218);
  ...
  display() {
    if (this.greyToggle != greyScale) {
      ...
      if (greyScale) {
        ...
        this.neckColor2 = color(102)
        this.feetColor = color(102);
        this.legColor = color(221);
        this.tailColor = color(102);
        this.eyeRimColor = color(238);
      }
      if (!greyScale) {
        this.headColor = color(255, 0, 0);
        this.crownColor = color(255, 0, 0);
    ...
    if (this.flying && this.mainBird) {
      ...
      this.birdPecking = false;
      if (this.flyingDir) {
        this.rootLoc.y -= walkSpeed; // was 12
      } else {
    ...
    push()
    translate(67, 0, 0); // beak 
    fill(this.beakColor);
    if (this.bw) {
      noFill();
    }
    rotateZ(-HALF_PI + .3) // rotate beak down
    cone(10, 45, this.detail, false);// beak size scale

    push();
    translate(-8, 0, 0) // lower beak
    rotateZ(-HALF_PI + 1.3) // rotate lower beak
    cone(7, 35, this.detail, false); // beak size scale
    pop();
    ...
    translate(30, -4, -this.headsize + 12) // left eye
    rotateY(-.35)
    ...
    pop()
    push() // crown 
    translate(-40, -this.headsize / 2 - 20, 0);
    rotateZ(-4.2);
