class ncardinal {
  constructor(_w, _YRot, _x, _y, _z) {
    ...
    this.wingLength = 200;
    this.wingWidthUp = 20;
    this.headColor = color(210, 180, 140);
    ...
    this.bodyColor = color(210, 180, 140); 
    this.wingColor = color(242, 120, 99);
    this.neckColor = color(105, 89, 92);
    this.feetColor = color(242, 120, 99);
    this.legColor = color(174, 159, 162);
  ...
  display() {
    if (this.greyToggle != greyScale) {
      this.greyToggle = greyScale;
      if (greyScale) {
        this.headColor = color(187);
        this.crownColor = color(68);
        this.cheekColor = color(170);
        this.beakColor = color(170);
        this.eyeColor = color(46);
        this.torusColor = color(143);
        this.bodyColor = color(187);
        this.wingColor = color(153);
        this.neckColor = color(136);
        this.feetColor = color(153);
        this.legColor = color(187);
        this.tailcolor = color(136);
        this.neckColor2 = color(187);
        this.backColor = color(85);
      }
      if (!greyScale) {
        this.headColor = color(210, 180, 140);
        this.crownColor = color(242, 120, 99);
    ...
    translate(-this.bodySize / 2.5, -this.bodySize / 4, 0);
    fill(this.bodyColor)
    if (this.bw) {
      noFill();
    }
    ellipsoid(this.bodySize * .5, this.bodySize * 1, this.bodySize * .6, this.detail, this.detail)  // draw body
    ...
    push()
    translate(33, 0, 0)  // beak
    fill(this.beakColor) 
    if (this.bw) {
      noFill();
    }
    rotateZ(-HALF_PI + .3) // rotate beak down
    cone(12, 30, this.detail, false); // beak size scale

    push();
    translate(-8, 0, 0) // lower beak
    rotateZ(-HALF_PI + 1.3) // rotate lower beak
    cone(9, 25, this.detail, false); // beak size scale 
    pop();
    ...
    translate(10, -4, this.headsize - 15) // right eye 
    ...
    translate(10, -4, -this.headsize + 15) // left eye
    torus(5, 2, torusDetail, torusDetail);
    fill(this.eyeColor) // pupil
    noStroke();
    if (this.bw) {
    ...
    pop();
    push(); // crown 
    translate(-53, -this.headsize / 2 - 12, 0);
    rotateZ(-4.2);
    ...
    this.drawWing(3.351, 0.6, 6.05, true) // rotation, separate function for wings because there's two of them 
    pop()
    push() // right wing
    translate(-80, -10, -this.bodySize * 0.6) // left wing
    this.drawWing(3.351, 3.0, 6.05, false)
    pop()
