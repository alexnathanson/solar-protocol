class raven {
  constructor(_w, _YRot, _x, _y, _z) {
    ...
    this.wingLength = 200;
    this.wingWidthUp = 20;
    this.headColor = color(0);
    this.bodyColor = color(0);
    this.wingColor = color(0);
    this.neckColor = color(0);
    this.feetColor = color(86, 86, 86);
    this.legColor = color(0);
    this.tailColor = color(0);
    this.eyeRing = color(16, 105, 192)
    this.uBeakColor = color(86, 86, 86)
    this.lBeakColor = color(120, 120, 120)
  ...
  display() {
    if (this.greyToggle != greyScale) {
      ...
      if (greyScale) {
        ...
        this.bodyColor = color(17);
        this.wingColor = color(17);
        this.neckColor = color(17);
        this.feetColor = color(86);
        this.legColor = color(17);
        this.tailColor = color(17);
        this.eyeRing = color(153);
        this.uBeakColor = color(85);
        this.lBeakColor = color(102);
      }
      if (!greyScale) {
        this.headColor = color(0);
        this.bodyColor = color(0); 
    ...
    if (this.birdPecking && this.mainBird) { // if it's pecking 
      ...
      if (this.bodyTilt < this.bodyTiltHome) {
        this.peckSpeed *= -1;
        this.birdPecking = false; // turn back on? 
    ...
    translate(-this.bodySize / 5, -this.bodySize / 3, 0); // center of body
    fill(this.bodyColor)
    if (this.bw) {
      noFill();
    }
    ellipsoid(this.bodySize * .6, this.bodySize * 1, this.bodySize * .7, this.detail, this.detail)  // draw body 
    ...
    rotateZ(this.bodyTilt + 5.3); // tilt of the whole body
    ...
    translate(67, -10, 0)  // beak
    fill(this.uBeakColor);
    if (this.bw) {
      noFill();
    }
    rotateZ(-HALF_PI + .05) // rotate beak down
    cone(10, 70, this.detail, false);// beak size scale
    // nose ellipsoid
    if (this.bw) {
      noFill();
    }
    push();
    translate(-10, 0, 0); // lower beak
    rotateZ(-HALF_PI + 1.3); // rotate lower beak
    fill(this.lBeakColor);
    if (this.bw) {
      noFill();
    }
    cone(8, 70, this.detail, false); // beak size scale
    pop();

    rotateZ(HALF_PI - .2)
    translate(-36, -10, 0) // nose ellipsoid
    pop();
    push();
    fill(this.eyeRing);
    stroke(this.eyeRing); // torus
    if (this.bw) {
      noFill();
      stroke(255);
    }
    ...
    sphere(4, this.detail, this.detail)
    pop();

    push();

    fill(this.eyeRing) // fill torus
    stroke(this.eyeRing) // fill torus
    if (this.bw) {
      noFill();
      stroke(255);
    }
    ...
    if (this.mainBird) {
      ...
      if (!primePeckingBuffer) {
        if (this.flying) {
          ...
          rotateZ(sin(this.walkRotSpeed + _rotOffset) / 2 + .15); // how far the hip turns 
        }
      } else {
        rotateZ(0.15);
      }
