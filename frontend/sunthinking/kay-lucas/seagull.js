class seagull {
  constructor(_w, _YRot, _x, _y, _z) {
    ...
    this.wingColor = color(100, 100, 100);
    this.neckColor = color(30, 30, 30);
  ...
  display() {
    ...
    if (this.mainBird) {
      this.walkRotSpeed += map(walkSpeed, 0, 6, 0, 0.1); // set the bird steps to match the walking speed was
    }
    else {
    ...
    ellipsoid(this.bodySize * .6, this.bodySize * 1.2, this.bodySize * .7, this.detail, this.detail)
    ...
    push()
    translate(this.headsize - 4, 8, 0) // edge of beak   
    rotateZ(this.laughingRot / 4);
    translate(28, 0, 0) // beak 
    fill(this.beakColor);
    if (this.bw) {
      noFill();
    }
    rotateZ(-HALF_PI) // rotate beak down
    cylinder(5, 60, this.detail, false);// beak size scale  
    pop()
    push()
  ...
  drawLeg(_rotOffset) {
    ...
    pop();
    translate(0, 24, 0); // right toe center
    cylinder(3, 80);
    pop();
    push();
    rotateX(0.7);
    translate(0, 20, 0); // right toe
    cylinder(3, 50);
    pop();
    push();
    rotateX(-0.7);
