...
let baseURL = 'https://solarprotocol.net:' + port + '/api/v2/opendata.php?';

let ignoreNames = ['woodbine','swarthmoresolar'];
let ignorePositions = [];
...
function gotServerData(tempdata){
  ...
  print(serverNames)
  ignorePositions = getNamePos(serverNames)
  print(ignorePositions)
  ...
  drawnServers = 0;//amtServers - ignoreNames.length; //the amount of servers to draw

  for(let s=0;s< amtServers;s++){
    if (ignorePositions.includes(s)){
      continue;
    }
    ...
    if(sD != 'err'){
      ...
      drawServerData(adjustedKeys,drawnServers);
      drawnServers = drawnServers + 1;
    }
  }
    drawLabels();
}
...
function gotPOEData(tempdata){
  ...
  let poeIgnorePos = getNamePos(tempdata['dump']['name'])
  print(tempdata['dump']['name'])
  //adjust time zones
  let adjustedPOE = [];
  ...
  drawPOEData(remPOE,poeIgnorePos);
}
...
//returns the position(s) of the names in an array, in an array
function getNamePos(data){
  nP = []
  for (let n = 0; n < data.length; n ++){
    //remove spaces and make lower case
    if (ignoreNames.includes(data[n].toLowerCase().replace(/ /g,""))){
      nP.push(n)
    }
  }
...
function drawServerData(data, pos){
  ...
  let yRange = (height - (2*yMargin))/(amtServers-ignoreNames.length);
...
function drawPOEData(data,iP){
  ...
  for (let x=0;x < data.length; x++){
    ...
    //width of rect
    if (x >= 1){
      drawXw =parseInt(map(new Date(data[prevX][0]).getTime(), mostRecent, farthest, width-xMargin, xMargin));
    ...
    //this is needed to adjust the positioning for filtering out names, because this code I wrote is overly complicated...
    let yP = data[x][1];
    ...
    if(drawX >= xMargin){
      ...
      rect(drawXw, yMargin + (yRange*yP),drawX, yMargin + yRange + (yRange*yP));

      //mark start of POE period
      stroke(0,0,255);
      line(drawX,yMargin + (yRange*yP),drawX,yMargin + (yRange*yP)+ (yRange))
    } else if (drawX < xMargin && drawXw >= xMargin ){
      rect(drawXw, yMargin + (yRange*yP),xMargin, yMargin + yRange + (yRange*yP));
    }
...
function drawLabels(){
  ...
  for(let s = 0; s < amtServers; s++){
    ...
    line(xMargin-10,yMargin + (yRange*drawnPos),width - xMargin,yMargin + (yRange*drawnPos));
    text(serverNames[s],yAx - 5,(yMargin + (yRange*drawnPos))+(yRange*.5));

    if(timezoneArr[s] != null){
      let tzCity = timezoneArr[s].split("/")[1];
      text("tz: " + tzCity ,yAx - 5,(yMargin + (yRange*drawnPos))+(yRange*.5)+25);
    }
    drawnPos = drawnPos + 1;
  }
